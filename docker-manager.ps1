...
function Init-Database {
    ...
    if (-not (Test-Path $sqlFile)) {
        ...
        Write-Error "Please create schema.sql in the project root directory"
        return
    }

    Write-Step "Copying SQL file to container" -Color Yellow
    docker cp $sqlFile mysql-demo:/tmp/schema.sql
    Write-Success "SQL file copied"

    Write-Step "Executing schema.sql" -Color Yellow
    docker exec -e MYSQL_PWD=123456 mysql-demo mysql -u root -e "source /tmp/schema.sql" 2>&1
