...
public class WechatPayClient implements PayClient {
    ...
    private final String apiV3Key;      // 正式或沙箱密钥
    private final String apiBaseUrl;    // 已包含 /sandboxnew 前缀
    ...
    public ThirdPartyPayResponse unifiedOrder(ThirdPartyPayRequest request) {
        ...
        try {
            ...
            String responseBody = restClient.post()
                    .uri(apiBaseUrl+"/v3/pay/transactions/jsapi")
    ...
    public QueryOrderResponse queryOrder(QueryOrderRequest request) {
        // 构建请求 URL（微信支付查询订单接口）
        String url = apiBaseUrl + "/v3/pay/transactions/out-trade-no/" + request.getPaymentId() + "?mchid=" + mchId;
    ...
    private String mapTradeState(String wechatState) {
        ...
        switch (wechatState) {
            ...
            case "PAYERROR":   // 🔥 明确映射为 FAILED
                return "FAILED";
            default:
                log.error("未知支付状态告警: {}", wechatState);
                // 发送告警通知（钉钉/企微/邮件），让开发人员立即知道微信支付新增了状态码
                alertService.sendAlert("微信支付未知状态", "状态码: " + wechatState);
                throw new BusinessException(ResultCode.THIRD_PARTY_ERROR);
        }
    }
