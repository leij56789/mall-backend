...
/**
 * 支付超时消费者
 * 监听 payment.timeout.queue，处理超时取消
 */
@Slf4j
@Component
...
public class PaymentTimeoutConsumer {
    ...
    @Log("支付单超时消息消费者")
    @RabbitListener(queues = RabbitMQConfig.PAYMENT_TIMEOUT_QUEUE)
    public void handlePaymentTimeout(Message message, Channel channel) throws IOException {
        log.info("支付单超时消费者");
...
//        log.info("Redis increment 结果：key={}, value={}", retryKey, retryCount);
        ...
        PaymentTimeoutMessage paymentTimeoutMessage = null;
        try {
            String body = new String(message.getBody(), StandardCharsets.UTF_8);
            paymentTimeoutMessage = objectMapper.readValue(body, PaymentTimeoutMessage.class);
        } catch (Exception e) {
            log.warn("业务异常，确认消息:messageId={},message={}", messageId, e.getMessage());
        ...
        if (retryCount > maxRetry) {
            ...
            alertService.sendAlert("消息消费超过最大重试次数",
                    String.format("messageId=%s, retryCount=%d, maxRetry=%d",
                            messageId, retryCount, maxRetry));
            return;
        }
        String paymentId = null;
        try {
            paymentId = paymentTimeoutMessage.getPaymentId();
            if (paymentId == null) {

            }
            paymentOrderService.cancelTimeoutPayment(paymentTimeoutMessage);
            channel.basicAck(deliveryTag, false);
            log.info("消费成功，paymentId={},messageId={}", paymentId, messageId);
            redisTemplate.delete(retryKey);
//            throw new RuntimeException("模拟消费失败，触发死信转发测试");
        } catch (BusinessException e) {
            Integer code = e.getCode();
            if (code.equals(ResultCode.STOCK_RECOVER_FAIL.getCode())) {
                log.warn("乐观锁冲突，消息将延迟重试：paymentId={},messageId={},errorCode={},message={}", paymentId, messageId, code, e.getMessage());
                channel.basicNack(deliveryTag, false, true);
            } else {
                log.warn("业务异常，确认消息:paymentId={},messageId={},errorCode={},message={}", paymentId, messageId, code, e.getMessage());
                clearRetryKey(retryKey);
                channel.basicAck(deliveryTag, false);
            }
        } catch (Exception e) {
//            log.info("key={},value={}",retryKey,redisTemplate.opsForValue().get(retryKey));
            log.error("系统异常，消息将延迟重试：paymentId={},messageId={},retryCount={}", paymentId, messageId, retryCount,e.getMessage());
            channel.basicNack(deliveryTag, false, true);
        }
    }
